...
partial map.  The second checkpoint adds structural totality and realization:
embedded values are total and realize their sources, realization is downward
closed, and the three colist cases behave as intended.  Finite requests and
operational coverage are now the next checkpoint.
...
For the structurally lifted order, no second coinductive definition is
needed.  The prototype validates the exact equation

```text
d realizes v  :=  d ⊑ embed v
```

because `nu_le` already says that a hole makes no claim and that two returned
layers must have the same semantic shape and pointwise-related children.  This
reuse is specific to the operational order generated by `Lift C`; it does not
identify realization with an arbitrary intrinsic semantic order.
...
Structural totality is defined coinductively: a total value exposes a returned
semantic layer and every recursive child is total.  Finite observations should
characterize this predicate by proving `Total d ↔ ∀ r, Observes r d`; they
should not introduce a second meaning of totality.  Totality is not defined as
...
Milestone 2M's carrier split, structural totality, and realization bridge now
pass.  The remaining experiments are:

1. Define colist requests and coverage, prove that request-indexed totality
   characterizes structural `Total`, keep it distinct from
   “produces at least `k` cons cells,” and prove evaluator-stage soundness
   separately from monotonicity.
2. Compare direct `ν C` semantic values with the total subtype of `Partial C`
   before fixing the final public representation.
3. Repeat the observation account for Boolean cotrees, where a request is a
   finite prefix-closed frontier rather than merely a scalar depth.
4. Instantiate the event container for `zar`'s Boolean interaction trees and
...
5. Use the interaction-tree slice to expose the precise finitary-event
   boundary and to decide whether lifting needs reified signature syntax.
6. Reconsider a direct sum/product backend or extraction-specific
